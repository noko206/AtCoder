...
function o(...$a) { if(count($a) === 1) { if(is_array($a[0])) { echo implode("\n", $a[0]); } else { echo $a[0]; } echo "\n"; } else { echo implode(' ', $a), "\n"; }}
...
function main() {
	...
	// メイン処理
	$a = array_map(function ($k, $v) { return [$v, $k]; }, array_keys($a), $a);
	rsort($a);
	$dp = array_fill(0, $n + 1, array_fill(0, $n + 1, -INF64));
	$dp[0][0] = 0;
	for ($i = 0; $i <= $n; ++$i) {
		list($v, $k) = $a[$i];
		for ($j = 0; $j <= $i; ++$j) {
