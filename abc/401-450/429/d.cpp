...
// clang-format on
template <typename T>
pair<map<T, int>, vector<T>> compress(const vector<T>& a) {
    auto decomp = a;
...
int main() {
    ...
    ll ans = 0;
    ll left = 0;
    for (auto [dpos, cpos] : comp) {
        if (dpos == 0) continue;
        if (dpos >= m + 1) break;  // mまでやる
        int ok = cn;
        int ng = cpos;
        while (abs(ng - ok) != 1) {
            int mid = (ok + ng) / 2;
            if (cs.sum(cpos, mid) >= c) {
                ok = mid;
            } else {
                ng = mid;
            }
        }
        ans += cs.sum(cpos, ok) * (dpos - left);
        left = dpos;
    }
    output(ans);
}
