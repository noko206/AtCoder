...
function main()
{
	...
	// 座標圧縮
	list($compress, $decompress) = compress($a);

	// クエリを処理
	$bt = new Binary_Trie();
	$ans = [];
	foreach ($queries as list($t, $x, $k)) {
		$x = $compress[$x]; // 座標圧縮
		if ($t === 1) {
			$bt->insert($x);
		} elseif ($t === 2) {
			$rank = $bt->upper_bound($x) - 1;
			if ($rank < $k) {
				$ans[] = -1;
			} else {
				$ans[] = $decompress[$bt->kth_elm($rank - $k + 1)];
			}
		} else {
			$rank = $bt->lower_bound($x);
			if ($bt->size() - $rank + 1 < $k) {
				$ans[] = -1;
			} else {
				$ans[] = $decompress[$bt->kth_elm($rank - 1 + $k)];
			}
		}
	}
