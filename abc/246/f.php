...
function main()
{
	...
	// 前処理
	$alpha_to_num = array_flip(range('a', 'z'));
	$set = array_fill(0, $n, 0);
	for ($i = 0; $i < $n; ++$i) {
		for ($j = 0; $j < strlen($s[$i]); ++$j) {
			$set[$i] |= 1 << $alpha_to_num[$s[$i][$j]];
		}
	}

	// bit全探索
	$ans = 0;
	$mint = new Modint(Modint::MOD998244353);
	for ($bit = 1; $bit < (1 << $n); ++$bit) {
		...
		if (__builtin_popcount($bit) % 2 === 0) {
			$mint->chsub($ans, $mint->pow(__builtin_popcount($cnt), $l));
		} else {
			$mint->chadd($ans, $mint->pow(__builtin_popcount($cnt), $l));
		}
	}
