...
function main()
{
    ...
    $s = str_split($s); // 各文字を分割して配列にする
    $t = str_split($t); // 各文字を分割して配列にする

    $roma = array_flip(range('a', 'z')); // 英小文字を数字に変換する配列
    $n = count($s);

    for($i = 0; $i < $n; ++$i)
    {
        $s[$i] = $roma[$s[$i]]; // 英小文字を数字に変換
        $t[$i] = $roma[$t[$i]]; // 英小文字を数字に変換
    }

    $k = ($t[0] - $s[0] + 26) % 26; // kの候補を一つ選ぶ
    $is_ok = true;
