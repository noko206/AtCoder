...
vector<vector<T>> matrix_pow(vector<vector<T>> a, long long n) {
    ...
    for (int i = 0; i < a.size(); i++) res[i][i] = (1LL << 32) - 1;
    while (n > 0) {
...
int main() {
    ...
    REP(i, k) cin >> a[i];
    REP(i, k) cin >> c[i];
    vector A(k, vector<ll>(k, 0));
    REP(i, k) A[0][i] = c[i];
    REP(i, k - 1) A[i + 1][i] = (1LL << 32) - 1;
    auto B = matrix_pow(A, m - 1);
    vector C(k, vector<ll>(1));
    REP(i, k) C[i][0] = a[k - i - 1];
    auto D = matrix_mul(B, C);
    output(D[k - 1][0]);
}
