...
int main() {
    ...
    vector<string> s(m);
    REP(i, m) cin >> s[i];
    cumulative_sum_2d<int> csj(m, n), cso(m, n), csi(m, n);
    REP(i, m) {
        REP(j, n) {
            if (s[i][j] == 'J') csj.add(i, j, 1);
            if (s[i][j] == 'O') cso.add(i, j, 1);
            if (s[i][j] == 'I') csi.add(i, j, 1);
        }
    }
    csj.build();
    cso.build();
    csi.build();
    vector<tuple<int, int, int>> ans(k);
    REP(i, k) {
        int sy, sx, gy, gx;
        cin >> sy >> sx >> gy >> gx;
        --sy, --sx;
        int ans_j = csj.sum(sy, sx, gy, gx);
        int ans_o = cso.sum(sy, sx, gy, gx);
        int ans_i = csi.sum(sy, sx, gy, gx);
        ans[i] = {ans_j, ans_o, ans_i};
    }
    for (auto [a, b, c] : ans) {
        output(a, b, c);
    }
